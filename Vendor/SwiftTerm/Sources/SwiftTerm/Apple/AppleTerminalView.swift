...
extension TerminalView {
    ...
    /// Synchronizes the terminal grid and the backing PTY with the view's current frame.
    ///
    /// AppKit normally reaches this through `setFrameSize(_:)`, but apps that keep
    /// terminal views alive while moving them between containers can reattach after
    /// the child process has already drawn its first TUI frame. This explicit hook
    /// lets the host app force a final size pass once the view is in its real window.
    @discardableResult
    public func synchronizeSizeWithCurrentFrame(forceNotify: Bool = false) -> Bool {
        guard cellDimension != nil else { return false }
        let changed = processSizeChange(newSize: frame.size)
